...
client = LINE("EtnpjrFjyshObq8dext5.IqtJipTzyCWvcrgownX/bq.D7UmVPJfbdYlWBnSlLsVWn4lgzz189R0dEKS2IpIXVg=")
clientMid = client.profile.mid
clientProfile = client.getProfile()
clientSettings = client.getSettings()
clientPoll = OEPoll(client)

ki = LINE("EtA2JUPj4sGRCZSFBgjf.aXKsmGph9h8IqosquepKZW.kgcMDTid1sUtkFdOKSSbeHwFy5cmV6GSzsRkD2FX2JY=")
    ...
    try:
        ...
        if op.type == 5:
            ...
            if settings["autoAdd"] == True:
                sendMention(op.param1, "Hi @! Thanks for add !", [op.param1])
...
# Pembatas Script #   
                                ...
                                if msg._from in admin:
                                   try:
                                       ...
                                       logError(error)
                            elif text.lower().startswith("ytmp4 "):
                                if msg._from in admin:
                                   try:
                                       link = text.lower().replace('ytmp4 ', '')
                                       ...
                                       for data in data['result'].get('videolist', []):
                                           if "720p" in data['resolution']:
                                               ki.sendVideoWithURL(msg.to,data['url'])
                                           else:
                                               pass
                                   except Exception as e:
                                       ki.sendMessage(msg.to,str(e))
                                ...
                                dan = text.split("|")
                                x = ki.findContactsByUserid(dan[1])
                                a = ki.getContact(sender)
                                sendMention(x.mid,"Anda mendapatkan pesan dari @! \n\n"+dan[2][a.mid])
                                sendMention(to,"Sukses mengirim pesan ke "+x.displayName+"\nDari: @! \nPesan: "+dan[2][a.mid])
                            elif text.lower().startswith("sendmsg"):
                                if msg._from in admin:     
                                    dan = text.split("|")
                                    x = ki.findContactsByUserid(dan[1])
                                    a = ki.getContact(sender)
                                    ki.sendMessage(x.mid,"Anda mendapatkan pesan dari "+a.displayName+"\n\n"+dan[2])
                                    ki.sendMessage(to,"Sukses mengirim pesan ke "+x.displayName+"\nDari: "+a.displayName+"\nPesan: "+dan[2])
# Pembatas Script #
# Pembatas Script #
...
# Pembatas Script #
                        ...
                        if msg._from in admin:
                            if settings["ChangeVideoProfilevid"] == True:
                               ...
                               pictures = "tmp/1527653274428-picsay.jpg"
                               client.sendMessage(to, "[ Profile ] \nType: Change Profile Video Picture\nStatus: Waiting...")
                        ...
                        if "/ti/g/" in msg.text.lower():
                            if settings["suf"] == True:
                                ...
                                for ticket_id in n_links:
                                    ...
                                    break
                        if "/ti/p/~" in msg.text.lower():
                            link_re = re.compile('(?:line\:\/|line\.me\/R)\/ti\/p\/~([a-zA-Z0-9_-]+)?')
                            links = link_re.findall(text)
                            n_links = [link for link in links]
                            for id in n_links:
                                contact = ki.findContactsByUserid(id)
                                ki.sendContact(receiver, contact.mid)
                                break
            except Exception as error:
                logError(error)
                traceback.print_tb(error.__traceback__)
